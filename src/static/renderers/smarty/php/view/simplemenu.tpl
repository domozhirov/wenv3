    ...
    <table{if $class} class="{$class}"{/if}>
        ...
        <td><ul><li{if !empty($item_classes) && $item_classes.0 != ""} class="{foreach from=$item_classes item=e2 name=n2}{$e2}{if !$smarty.foreach.n2.last} {/if}{/foreach}"{/if}>
                ...
                <li{if !empty($item_classes) && $item_classes.0 != ""} class="{foreach from=$item_classes item=e2 name=n2}{$e2}{if !$smarty.foreach.n2.last} {/if}{/foreach}"{/if}>
                    {/if}
                    <a href="{$data_url}"{if $e.blank} target="_blank"{/if}>
                        {if $img_width && $img_height && $e._level<=$img_enable_to_level && $e.image.filename}
                            <span class="{$item_classes.3|default:"item"}-image"><img src="{s3_img src=$e.image.filename width=$img_width height=$img_height method=$img_method|default:'r'}" alt="{$name|escape}"></span>
                        {/if}
                        {if $item_classes.3}
                        <span class="{$item_classes.3|default:"item"}-text">
			{/if}
                            {section loop=$span name=sp}<span>{/section}{$name}{section loop=$span name=sp}</span>{/section}
                            {if $item_classes.3}
			</span>
                        {/if}
                    </a>
    ...
    <ul{if $class} class="{$class}"{/if}>
        ...
        {foreach from=`$data` item=e key=k name=pro}
            ...
            {else}
                {capture assign=data_url}{$e.page_url}?mode=folder&amp;folder_id={$e.folder_id}{/capture}
            {/if}
                {if $folder.folder_id == $e.folder_id}
                    {assign var=active value=1}{else}{assign var=active value=0}{/if}
                {if $folder._left>=$e._left && $folder._right<=$e._right}{assign var=opened value=1}{else}{assign var=opened value=0}{/if}
                {assign var=name value=$e.folder_name}
                {if $data.0._level<1}{assign var=k value=$k-1}{/if}
            {else}
    ...
    {elseif $type == 4}
        ...
        *******}
        ...
        {if !empty($data)}
            ...
            {if $collevel==1}
                {foreach from=$data item=e}
                    {if $e._level > 0 && ($e._level==1 || $e.vendor_id > 0)}
            ...
            {else}
                ...
                {if $test_child}
                    ...
                    {assign var=data_childs value=" "|explode:""}
                    ...
                    {foreach from=$data item=e key=k}
                        {if $e._level > 0}
                            ...
                            {elseif $e._level==$collevel}
                                {assign var=parent value=$data_childs.$parent_index}
                                {assign var=parent value=$parent+1}
                                {assign_hash var=data_childs.$parent_index value=$parent}
                            {/if}
                        {/if}
                    {/foreach}
                {else}
            ...
            {foreach from=$data item=e key=k name="foo"}
                ...
                {if $e._level>0}
                    ...
                    {if $e._level+1==$collevel}
                        {if $test_child}
                            {assign var=data_child value=$data_childs.$k|default:0}
                            {assign var=ceil_amount value=$data_child/$cols}
                            {assign var=ceil_amount value=$ceil_amount|floor}
                            {assign var=additional_value value=$data_child%$cols}
                    ...
                    {if $opened && $openclass}{assign_hash var=item_classes.1 value=$openclass}{/if}
                    {if $active && $activeclass}{assign_hash var=item_classes.2 value=$activeclass}{/if}
                    <li{if !empty($item_classes) && $item_classes.0 != "" || $item_classes|@count>1} class="{foreach from=$item_classes item=e2 name=n2}{if $e2}{$e2}{if !$smarty.foreach.n2.last} {/if}{/if}{/foreach}"{/if}>
                    <a href="{$data_url}" {if $e.blank} target="_blank"{/if}>
                        {if $img_width && $img_height && $e._level<=$img_enable_to_level && $e.image.filename}
                            <span class="{$item_classes.3|default:"item"}-image"><img src="{s3_img src=$e.image.filename width=$img_width height=$img_height method=$img_method|default:'r'}" alt="{$name|escape}"></span>
                        {/if}
    ...
    {else}
        ...
        *******}
        {capture assign=outputmenu}
            ...
            {if $item_classes}
                {assign var="item_custom_classes" value=","|explode:$item_classes}
            {/if}

            {if $delimiters}
                {assign var="delimiters" value=","|explode:$delimiters}
            {/if}
        ...
        {foreach from=`$data` item="e" name="foo"}
            ...
            {/if}
                {if $folder}
                    ...
                    {* Если главная магазина1 *}
                    {if $page.plugin_id == 1 && !$mode}
                    ...
                    {else}
                        ...
                        {if $folder._left>=$e._left && $folder._right<=$e._right }
                            {assign var=opened value=1}
                        {else}
                            {assign var=opened value=0}
                        {/if}
                    {/if}
        ...
        {if $e._level>0 && ( ($e._left > $fromparent._left  && $e._right < $fromparent._right) || !$subfrom) && (!$tolevel || $e._level < $tolevel-$fromlevel)}
            ...
            {/if}
            {assign var=item_classes value=" "|explode:""}
            {assign var=item_index value=$e._level-1}
            {if $has_child && $haschildclass}{assign_hash var=item_classes.0 value=$haschildclass}{/if}
            {if $opened && $openclass}{assign_hash var=item_classes.1 value=$openclass}{/if}
            {if $active && $activeclass}{assign_hash var=item_classes.2 value=$activeclass}{/if}
            {if $item_custom_classes.$item_index}
                {assign_hash var=item_classes.3 value=$item_custom_classes.$item_index}
            {/if}
            {if $key == 0 && $home_class}
                <li class="{$home_class}"><a href="/"></a></li>
            {/if}
            {if $delimiters && (($e._level == 1 && $delimiters.0 != '') || ($e._level > 1 && $delimiters.1 != '' && $e._level <= $prev_level)) && (($key == 0 && $home_class) || $key)}
                <li class="{if $delimiters.0}{$delimiters.0}{elseif $delimiters.1}{$delimiters.1}{/if}"></li>
            {/if}
            <li{if !empty($item_classes) && $item_classes.0 != "" || $item_classes|@count>1} class="{foreach from=$item_classes item=e2 name=n2}{if $e2}{$e2}{if !$smarty.foreach.n2.last} {/if}{/if}{/foreach}"{/if}>
            <a href="{$data_url}" {if $e.blank} target="_blank"{/if}>
                {if $img_width && $img_height && $e._level<=$img_enable_to_level && $e.image.filename}
