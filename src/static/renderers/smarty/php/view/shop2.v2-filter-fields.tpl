        ...
        {if $param._sph_filter}
            ...
            {if $param.type == 'int' || $param.type == 'float'}
				<tr class="shop2-filter-fields">
					...
					<td>
                        ...
                        {if $param.range}
							<label>
								<strong>{#SHOP2_FROM#}</strong> <input class="{$class}" type="text" value="{$smarty.get.s[$param._sph_name].min|htmlspecialchars}" name="s[{$param._sph_name}][min]"{include file="global:shop2.v2-facets-data.tpl" aggs=$aggs.cf field_name=$param._sph_name key="min" empty_class=$facets_empty_class  placeholder=true} />
							</label>
							<label>
								<strong>{#SHOP2_TO#}</strong> <input class="{$class}" type="text" value="{$smarty.get.s[$param._sph_name].max|htmlspecialchars}" name="s[{$param._sph_name}][max]"{include file="global:shop2.v2-facets-data.tpl" aggs=$aggs.cf field_name=$param._sph_name key="max" empty_class=$facets_empty_class  placeholder=true} />
							</label>
                        {else}
							<label>
            ...
            {elseif $param.type == 'checkbox'}
				<tr class="shop2-filter-fields">
					<th>{$name}:</th>
					<td>
						<a href="#" data-name="s[{$param._sph_name}][1]" data-value="1" class="param-val{if $smarty.get.s[$param._sph_name].1==1} active-val{/if}{include file="global:shop2.v2-facets-data.tpl" aggs=$aggs.cf field_name=$param._sph_name key=1 empty_class=$facets_empty_class empty_class_inset=true}">{#SHOP2_YES#}<span>&nbsp;</span></a>
						<a href="#" data-name="s[{$param._sph_name}][0]" data-value="0" class="param-val{if $smarty.get.s[$param._sph_name].0==0 && isset($smarty.get.s[$param._sph_name].0)} active-val{/if}{include file="global:shop2.v2-facets-data.tpl" aggs=$aggs.cf field_name=$param._sph_name key=0 empty_class=$facets_empty_class empty_class_inset=true}">{#SHOP2_NO#}<span>&nbsp;</span></a>
					</td>
				</tr>
