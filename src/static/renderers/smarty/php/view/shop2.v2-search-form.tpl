		...
		<form action="{get_seo_url uri_prefix=$shop2.uri mode="search"}" enctype="multipart/form-data">
			...
			{foreach from=$product_flag item=flag}
				{if ($flag.flag_id == 2 && !in_array("special", $hide_in_search)) || ($flag.flag_id == 1 && !in_array("new", $hide_in_search)) || ($flag.flag_id != '1' && $flag.flag_id != '2')}
				    <div class="row">
				        <div class="row-title">{$flag.name}:</div>
				            {assign var=flag_key value=$flag.flag_id}
				            <select name="s[_flags][{$flag.flag_id}]">
				            <option value=""{if $search_params._flags.$flag_key === ''} selected="selected"{/if}>{#SHOP2_ALL#}</option>
				            <option value="1"{if $search_params._flags.$flag_key === '1'} selected="selected"{/if}>{#SHOP2_YES#}</option>
				            <option value="0"{if $search_params._flags.$flag_key === '0'} selected="selected"{/if}>{#SHOP2_NO#}</option>
				        </select>
				    </div>
		    	{/if}
			{/foreach}
