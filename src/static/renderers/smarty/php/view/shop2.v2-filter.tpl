			...
			<table class="table-filter-param">
                {if !in_array("price", $hide_in_filter) && $show_price}
					...
					<tr class="type-text">
						...
						<td>
							<label>
								<strong>{#SHOP2_FROM#}</strong> <input class="shop2-input-float" type="text" value="{$smarty.get.s.price.min|htmlspecialchars}" name="s[price][min]"{include file="global:shop2.v2-facets-data.tpl" field_name="price" key="min" empty_class=$facets_empty_class placeholder=true}/>
							</label>
							<label>
								<strong>{#SHOP2_TO#}</strong> <input class="shop2-input-float" type="text" value="{$smarty.get.s.price.max|htmlspecialchars}" name="s[price][max]"{include file="global:shop2.v2-facets-data.tpl" field_name="price" key="max" empty_class=$facets_empty_class placeholder=true}/>
							</label>
			...
			<div class="shop2-sorting-panel">
				<div class="sorting">
                    ...
                    {if $shop2.my.show_amount_sort}
						<a href="#" class="sort-param {if $sort_amount_asc}sort-param-asc{elseif $sort_amount_desc}sort-param-desc{/if}" data-name="amount">{#SHOP2_AVAILABILITY#}<span>&nbsp;</span></a>
                    {/if}

                    {if $shop2.my.show_rating_sort}
						<a href="#" class="sort-param {if $sort_rating_asc}sort-param-asc{elseif $sort_rating_desc}sort-param-desc{/if}" data-name="rating">{#SHOP2_RATING2#}<span>&nbsp;</span></a>
                    {/if}
				...
				<div class="view-shop">
                    {assign var="l_shop2_view" value="SHOP2_VIEW"|l}
					<strong>{#SHOP2_VIEW#|default:$l_shop2_view}:</strong>
