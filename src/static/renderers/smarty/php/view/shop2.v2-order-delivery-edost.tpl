				...
				<div class="shop2-edost-variant">
					...
					<label>
						...
						{if $e.day}
							<div class="shop2-edost-day">
								{#SHOP2_ESTIMATED_DELIVERY_TIME#}: {$e.day}
							</div>
						{/if}
					</label>
					...
					{if $e.pickpointmap}
						...
						<div class="shop2-edost-pickpointmap">
							<a data-city="{$e.pickpointmap}" href="#">{#SHOP2_SELECT_POSTAMAT_OR_POINT_DELIVERY#}<span>{if $postamat.name}: {$postamat.name}{/if}</span></a>
					...
					{foreach from=$e.offices key=office_id item=office}
						<div class="shop2-edost-office">
							...
							{if $office.schedule}
								<div class="shop2-edost-office-schedule">{#SHOP2_WORK_HOURS#}: {$office.schedule}</div>
							{/if}
							
							{if $office.tel}
								<div class="shop2-edost-office-tel">{#SHOP2_PHONE#}: {$office.tel}</div>
							{/if}
						</div>
					{/foreach}
				</div>
