...
<form method="post" action="{$shop2.uri}?mode=cart&amp;action=add" accept-charset="utf-8" class="shop2-product-item product-item-thumb">
    ...
    <div class="product-top">
        <div class="product-image{if !$product.image_filename} no-image{/if}">
            ...
            {/if}
            {if $product.new || $product.special || $product_flag}
            <div class="product-label">
                {if $product.special}<div class="product-spec" style="margin: 0 2px 2px 0;">{$shop2.my.special_alias|default:#SHOP2_SPECIAL#}</div>{/if}
                {if $product.new}<div class="product-new" style="margin: 0 2px 2px 0;">{$shop2.my.new_alias|default:#SHOP2_NEW#}</div>{/if}
					{foreach from=$product.flags item=flag_id key=k name=foo}
						...
						{if $smarty.foreach.foo.iteration <= 3}
							{if $flag.flag_id != '1' && $flag.flag_id != '2'}
						        <div class="product-flag" style="background: {$flag.color};  margin: 0 2px 2px 0;">
						        	{$flag.name}
						        </div>
						    {/if}
					    {/if}
