...
<form class="shop2-product-item tr" method="post" action="{$shop2.uri}?mode=cart&amp;action=add" accept-charset="utf-8">
	<div class="td column-name">
		...
		{if $product.new || $product.special || $product.flags}
			<div class="product-label">
				...
				{foreach from=$product.flags item=flag_id key=k}
					...
					{if $k<3}
						{if $flag.flag_id != '1' && $flag.flag_id != '2'}
					        <div class="product-flag" style="background: {$flag.color};">
	...
	{if $show_price}
		...
		{if !$shop2.my.mode_catalog || $shop2.my.mode_catalog2}
