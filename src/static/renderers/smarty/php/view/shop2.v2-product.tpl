...
<form
					...
					{foreach from=$product.flags item=flag_id key=k name=foo}
						...
						{if $smarty.foreach.foo.iteration <= 3}
							{if $flag.flag_id != '1' && $flag.flag_id != '2'}
					        	<div style="background: {$flag.color};">{$flag.name}</div>
						    {/if}
					    {/if}
					{/foreach}
			    </div>
				...
				{foreach from=$product.params.images item=img key=k}
					<li>
						<a href="{$IMAGES_DIR}{$img.filename}">
							<img src="{s3_img width=$width height=$width src=$img.filename method=$shop2.my.s3_img_method}" alt="{$product.name|htmlspecialchars}" title="{$product.name|htmlspecialchars}" />
						</a>
						<div class="verticalMiddle"></div>
					</li>
				{/foreach}
	...
	<div class="product-side-r">
		...
		{if $product.has_params || !$shop2.my.hide_share}
			<div class="product-details">
				...
				{if !$shop2.my.hide_share}
					<div class="yashare">
						...
						<div class="yashare-auto-init" data-yashareL10n="{$site.lang}" data-yashareType="none" data-yashareQuickServices="{$shop2.my.share_options|default:"vkontakte,facebook,twitter,odnoklassniki,moimir,gplus"}" {if $product.image_filename}data-yashareImage="//{$site.site_domain}{$IMAGES_DIR}{$product.image_filename}"{/if}></div>
					</div>
				{/if}
